...
postgresql-doc postgresql-server-dev-all  postgresql-server-dev-9.4 libreadline6-dev libreadline6 lib32readline6 lib32readline6-dev makepasswd \
libclass-dbi-pg-perl libcompress-zlib-perl zlib-bin zlib1g-dev libio-compress-zlib-perl libghc-zlib-dev libssl-dev acl-dev libacl1-dev -y || { echo "${RED}FALHA AO INSTALAR DEPENDENCIAS ${CLOSE}"; exit 1; }
...
cd /usr/src
wget -c  http://wiki.douglasqsantos.com.br/Downloads/bacula/bacula-7.0.5.tar.gz || { echo "${RED}FALHA AO OBTER O BACULA ${CLOSE}"; exit 1; }

tar -xzvf bacula-7.0.5.tar.gz
cd bacula-7.0.5

CFLAGS="-g -Wall" ./configure --with-openssl=yes --enable-smartalloc --with-postgresql  --with-db-name=bacula --with-db-user=bacula --with-db-password=${DB_PASSWORD} --with-db-port=5432 --with-working-dir=${BACULA_WD} --with-pid-dir=${BACULA_PID} --with-logdir=${BACULA_LOG} --with-readline=/usr/include/readline  --disable-conio --enable-lockmgr --with-scriptdir=${BACULA_BASE}/scripts
...
aptitude install apache2 php5 php5-pgsql php5-gd php5-mcrypt sudo php5-cli -y
cd /var/www/
rm -rf index.html
mkdir bacula-web
cd bacula-web
...
cat << EOF > /etc/apache2/sites-available/bacula-web.conf
<VirtualHost *:80>
...
htpasswd -cdb /etc/apache2/access/bacula-htpasswd bacula nex@134*..!
a2dissite default
a2ensite bacula-web
